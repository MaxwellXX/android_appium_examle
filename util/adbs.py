...
__author__ = "xuxu"
# from https://github.com/gb112211/Adb-For-Test , thank the author
import os
import platform
import subprocess
import re
...
class ADB(object):
    ...
    def changeInputToYj(self):
        """
        修改输入法为一加输入法
        :return:
        """
        return self.shell("ime set com.baidu.input_yijia/.ImeService").stdout.read().strip()

    def changeInputToUni(self):
        """
        修改输入法为一加输入法
        :return:
        """
        return self.shell("ime set io.appium.android.ime/.UnicodeIME").stdout.read().strip()
    ...
    def input_enter(self):
        ...
        :return:
        """
        return self.shell("input keyevent 66").stdout.read().strip()
    ...
    def getPid(self, packageName):
        ...
        if system == "Windows":
            ...
            pidinfo = self.shell("ps -A| grep -w %s" % packageName).stdout.read()
